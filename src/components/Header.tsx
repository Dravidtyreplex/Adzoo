...
export default function Header() {
  ...
  const whatsappUrl = "https://wa.me/919639390951?text=Hi%20Adzoop%2C%20I%27d%20like%20to%20know%20pricing%20details.";

  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-white/90 backdrop-blur-xl border-b border-slate-200/80 transition-all duration-300 shadow-sm">
      <div className="max-w-7xl mx-auto px-6 h-[85px] flex items-center justify-between">
          ...
          >
            ...
            {dropdownOpen && (
              <div className="absolute top-full left-0 w-64 bg-white border border-slate-200 rounded-2xl p-2.5 shadow-2xl backdrop-blur-2xl animate-in fade-in slide-in-from-top-2 duration-200 space-y-1">
                ...
                <Link href="/contact?solution=digital-billboard" className="block px-4 py-2.5 rounded-xl text-xs font-semibold text-slate-700 hover:text-[#0052FF] hover:bg-slate-50 transition-all">
                  🌐 Digital Billboard Network
                </Link>
                <Link href="/contact?solution=cab-interior" className="block px-4 py-2.5 rounded-xl text-xs font-semibold text-slate-700 hover:text-[#0052FF] hover:bg-slate-50 transition-all">
                  🚕 Cab Interior Screen Ads
                </Link>
                <Link href="/contact?solution=bus-back" className="block px-4 py-2.5 rounded-xl text-xs font-semibold text-slate-700 hover:text-[#0052FF] hover:bg-slate-50 transition-all">
                  🚌 Transit Bus Back Media
                </Link>
              </div>
            )}
          </div>
        ...
        <div className="hidden lg:flex items-center gap-4">
          <a
            ...
            className="px-5 py-2.5 rounded-xl btn-brand-primary text-xs tracking-wide font-sans flex items-center gap-2 shadow-[0_4px_20px_rgba(0,82,255,0.25)]"
          >
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden bg-white border-b border-slate-200 px-6 py-6 space-y-4 font-sans shadow-xl animate-in slide-in-from-top-4">
          ...
          <div className="space-y-2 pl-3 border-l-2 border-[#0052FF]/30">
            <span className="text-xs font-mono text-[#0052FF] uppercase tracking-wider block font-bold">Solutions</span>
            <Link href="/#how-it-works" onClick={toggleMobileMenu} className="block text-sm text-slate-600 hover:text-[#0052FF]">Auto LED Advertising</Link>
            <Link href="/contact?solution=digital-billboard" onClick={toggleMobileMenu} className="block text-sm text-slate-600 hover:text-[#0052FF]">Digital Billboard Network</Link>
            <Link href="/contact?solution=cab-interior" onClick={toggleMobileMenu} className="block text-sm text-slate-600 hover:text-[#0052FF]">Cab Interior Screen Ads</Link>
          </div>
          ...
          <div className="pt-2">
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="w-full py-3 rounded-xl btn-brand-primary text-white font-bold text-sm flex items-center justify-center gap-2"
            >
              <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24"><path d="M.057 24l1.687-6.163c-1.041-1.804-1.588-3.849-1.587-5.946C.06 5.348 5.397.01 12.008.01c3.202.001 6.212 1.246 8.477 3.513 2.266 2.268 3.507 5.28 3.505 8.484-.004 6.657-5.34 11.997-11.953 11.997-2.005-.001-3.973-.504-5.719-1.465L0 24z"/></svg>
              Chat on WhatsApp
            </a>
